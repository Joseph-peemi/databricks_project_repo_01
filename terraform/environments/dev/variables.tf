...
variable "enable_network_hardening" {
  type    = bool
  default = false # dev typically doesn't need VNet injection; flip on if your landing zone requires it everywhere
}
...
variable "git_branch" {
  description = "dev tracks the working branch directly -- fastest iteration loop."
  type        = string
  default     = "main"
}
...
variable "scale_to_zero_enabled" {
  type    = bool
  default = true # cost over latency in dev
}
